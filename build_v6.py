...
for c in CATS:
    ...
    for src in buckets[c["key"]]:
        ...
        for it in src["items"][:8]:
            ...
            sec_items_html += f'<a class="item-card fade-up" href="{it.get("link", "#")}"><span class="item-title">{t}</span></a>'
...
INSPIRATION_SRC = r"C:\Users\user\Desktop\crawl4ai\rag_data\ai_assets\inspirations.json"
inspiration_html = ""
if os.path.exists(INSPIRATION_SRC):
    try:
        ...
        for it in ins[:24]:
            ...
            src = it.get("src", "")
            inspiration_html += f'<div class="insp-card"><div class="insp-src">{src}</div><div class="insp-text">{txt}</div></div>'
...
<p class="hero-sub">{sum(sum(s["count"] for s in buckets[k]) for k in buckets)} 條真實標題 / {len(SOURCES)} 個臺灣源 / 4 大分類 — 客戶問房,即時有答案。</p>
<div class="hero-meta">
